...
| timsTOF Pro | TDF `.d` (PASEF, ion mobility) | MassIVE **MSV000101607** | `Blank_Try.d` — TIMS-PASEF proteomics, ion-mobility (native TDF `.d`) | 0.44 GB (raw `.d`) | MassIVE: raw `.d` via `DownloadResultFile` (`f.MSV000101607/raw/Blank_Try.d/…`) |
...
| ACQUITY UPLC **PDA** | mzML | ProteoWizard `Reader_Waters_Test` | `QC_LCMS2-2_23_268-1-1.mzML` — the corpus's **UV / photodiode-array exemplar**: 8 `wavelength array` spectra over 210–400 nm with `electromagnetic radiation` (absorption) intensities, i.e. mzPeak's `wavelength_spectra` facet. The rare case of UV spectra carried inside an mzML. | 85 KB | `https://raw.githubusercontent.com/ProteoWizard/pwiz/master/pwiz/data/vendor_readers/Waters/Reader_Waters_Test.data/QC_LCMS2-2_23_268-1-1.mzML` (also local at `data/general-ms/waters-pda-uv/`) |
